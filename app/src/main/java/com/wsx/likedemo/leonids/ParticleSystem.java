package com.wsx.likedemo.leonids;
...
public class ParticleSystem {
    ...
    public ParticleSystem(ViewGroup parentView, Resources resources, int maxParticles, int[] drawableInts, long timeToLive) {
        ...
        if (drawableInts != null && drawableInts.length > 0) {
            for (int i = 0; i < mMaxParticles; i++) {
                Drawable drawable = resources.getDrawable(drawableInts[random.nextInt(drawableInts.length)]);
                if (drawable instanceof AnimationDrawable) {
                    AnimationDrawable animation = (AnimationDrawable) drawable;
                    mParticles.add(new AnimatedParticle(animation));
                } else {
    ...
    public ParticleSystem(Activity a, int maxParticles, int[] drawableInts, long timeToLive) {
        this((ViewGroup) a.findViewById(android.R.id.content), a.getResources(), maxParticles, drawableInts, timeToLive);
    }
